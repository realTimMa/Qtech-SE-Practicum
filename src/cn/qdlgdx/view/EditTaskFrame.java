 ...
 *
 */
public class EditTaskFrame extends JInternalFrame {
...
//			public void run() {
//				try {
//					UserEditTask frame = new UserEditTask();
...
//						&& taskCate.length() < 10 && taskCate.length() > 1 && taskJan.length() < 10
//						&& taskFeb.length() < 10 && taskMar.length() < 10 && taskApr.length() < 10
//						&& taskMay.length() < 10 && taskJun.length() < 10 && taskJul.length() < 10
//						&& taskAug.length() < 10 && taskSept.length() < 10 && taskOct.length() < 10
//						&& taskNov.length() < 10 && taskDec.length() < 10 && taskDept.length() < 10
//						&& taskDept.length() > 0 && taskPrcp.length() < 50 && taskPrcp.length() > 0
//						&& taskAsstt.length() < 10 && taskAsstt.length() > 0 && taskPfmc.length() < 50
//						&& taskGrade < 999999999 && taskWeight < 999999999) {
				int flag = AdminDao.updateTask(taskId, taskName, taskCate, taskJan, taskFeb, taskMar, taskApr, taskMay,
						taskJun, taskJul, taskAug, taskSept, taskOct, taskNov, taskDec, taskDept, taskPrcp, taskAsstt,
						taskPfmc, taskGrade, taskWeight);
				if (flag == 0) {
...
//				}
		...
		btnSub.setBounds(31, 355, 97, 23);
		getContentPane().add(btnSub);

		JButton btnExit = new JButton("\u53D6\u6D88");
		btnExit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				EditTaskFrame.this.setVisible(false);
			}
		});
		btnExit.setBounds(180, 355, 97, 23);
		...
		JButton btnNewButton = new JButton("\u6309\u8D1F\u8D23\u4EBA\u5BFC\u51FA");
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String taskPath = txtPath.getText().trim();
				AdminDao.delectTablePrcp();
				AdminDao.sumPrincipal();
				int flag = AdminDao.outputleader(taskPath);
				if (flag == 1) {
		...
		btnNewButton.setBounds(240, 391, 123, 23);
		getContentPane().add(btnNewButton);

		txtPath = new JTextField();
		txtPath.setBounds(99, 392, 102, 21);
		getContentPane().add(txtPath);
		txtPath.setColumns(10);

		JList list = new JList();
		list.setBounds(53, 409, 1, 1);
		getContentPane().add(list);

		JLabel lblNewLabel_21 = new JLabel("\u8DEF\u5F84\uFF1A");
		lblNewLabel_21.setBounds(31, 395, 58, 15);
		getContentPane().add(lblNewLabel_21);

		JLabel lblNewLabel_22 = new JLabel("\u6708\u4EFD\uFF1A");
		lblNewLabel_22.setBounds(31, 420, 58, 15);
		getContentPane().add(lblNewLabel_22);

		txtMonth = new JTextField();
		txtMonth.setBounds(99, 417, 102, 21);
		getContentPane().add(txtMonth);
		txtMonth.setColumns(10);

		JButton btnNewButton_1 = new JButton("\u6309\u6708\u4EFD\u5BFC\u51FA");
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String taskPathM = txtPath.getText().trim();
				int taskMonth = Integer.parseInt(txtMonth.getText().trim());
				AdminDao.delectTableMonth();
				AdminDao.sumMonth(taskMonth);
				int flag = AdminDao.outputMonth(taskPathM);
				if (flag == 1) {
					JOptionPane.showMessageDialog(null, "导出成功！");
				} else if (flag == -1) {
					JOptionPane.showMessageDialog(null, "导出失败！");
		...
		btnNewButton_1.setBounds(240, 416, 123, 23);
